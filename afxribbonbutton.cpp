...
CSize CMFCRibbonButton::DrawBottomText(CDC* pDC, BOOL bCalcOnly)
{
	...
	if (!rectMenuArrow.IsRectEmpty())
	{
		...
		// FIX MFC BUG and draw the arrow using a new rect, not the one that was sized incorrectly for hight DPI displays.
		// The new rect is just the button rect with no adjustments. This seems to work fine with the image drawing code and
		// the images are not stretched or anything weird like that.
		rectMenuArrow.left = m_rect.left;
		rectMenuArrow.right = m_rect.right;
		// END of bug fix.
